/***********************************************************************************************************************
   ...
   DESCRIPTION
	...
	Desc    		:	This store procedure is use to get PFS_SYS_PARAM_GROUP by id
	Create Date 	:	29 Dec 2011		- Created By  : PFS Generator v5.0
	Modified Date 	:	29 Dec 2011		- Modified By : PFS Generator v5.0
	Comments		:
	Sample Data 	:
***********************************************************************************************************************/
CREATE PROCEDURE [dbo].[uspPFS_SysParamGroupGet]
(
...
BEGIN
	...
	SELECT
		...
		WHERE
			(@iSysParamGroupID IS NULL OR pspg0.pfs_sys_param_group_id = @iSysParamGroupID) AND
			pspg0.is_visible = 1
		ORDER BY
			pspg0.INDEX_ORDER DESC
END
