/***********************************************************************************************************************
   ...
   DESCRIPTION
	...
	Desc    		:	This store procedure is use to get list of PFS_MODULE_OBJECT_MEMBER_USER_ACTION by id
	Create Date 	:	02 Jan 2012		- Created By  : PFS Generator v5.0
	Modified Date 	:	02 Jan 2012		- Modified By : PFS Generator v5.0
	Comments		:
	Sample Data 	:
***********************************************************************************************************************/
CREATE PROCEDURE [dbo].[uspPFS_ModuleObjectMemberUserActionList]
(
...
BEGIN
	...
	SELECT
		...
		WHERE
			...
			(@iModuleObjectMemberID IS NULL OR pmomua0.pfs_module_object_member_id = @iModuleObjectMemberID) AND
			(@iModuleObjectID IS NULL OR pmomua0.pfs_module_object_id = @iModuleObjectID) AND
			(@sKeyWords IS NULL)
