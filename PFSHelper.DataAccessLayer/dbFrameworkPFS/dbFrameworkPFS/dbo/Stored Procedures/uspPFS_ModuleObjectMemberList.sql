/***********************************************************************************************************************
   ...
   DESCRIPTION
	...
	Sample Data 	:
***********************************************************************************************************************/
CREATE PROCEDURE [dbo].[uspPFS_ModuleObjectMemberList]
(
	@sKeyWords VARCHAR(1280) = NULL,
	@iModuleObjectID INT = NULL ,
	@bIsWithApprovalProccess BIT = NULL 
)
AS
BEGIN
	...
	SELECT
		...
		WHERE
			...
			(@iModuleObjectID IS NULL OR pmom0.pfs_module_object_id = @iModuleObjectID) AND
			(@bIsWithApprovalProccess IS NULL OR pmom0.is_with_approval_proccess = @bIsWithApprovalProccess) AND
			(@sKeyWords IS NULL OR
			pmom0.member_code LIKE '%' + @sKeyWords + '%' OR
			pmom0.member_name LIKE '%' + @sKeyWords + '%' OR
			pmom0.member_descr LIKE '%' + @sKeyWords + '%')
			AND pmom0.IS_VISIBLE = 1
