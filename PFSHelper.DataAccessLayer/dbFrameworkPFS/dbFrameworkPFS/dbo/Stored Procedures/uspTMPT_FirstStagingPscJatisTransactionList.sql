...
/***********************************************************************************************************************
   ...
   DESCRIPTION
	...
	Sample Data 	:
***********************************************************************************************************************/
CREATE PROCEDURE dbo.uspTMPT_FirstStagingPscJatisTransactionList
(
...
BEGIN
	...
	WHERE
		...
		(@iFileImportID IS NULL OR tfspjt0.psc_file_import_id = @iFileImportID) AND
		(@bIsValid IS NULL OR tfspjt0.is_valid = @bIsValid) AND
		(@sKeyWords IS NULL OR
		tfspjt0.date LIKE '%' + @sKeyWords + '%' OR
		tfspjt0.branch_code LIKE '%' + @sKeyWords + '%' OR
		tfspjt0.branch_name LIKE '%' + @sKeyWords + '%' OR
		tfspjt0.cif LIKE '%' + @sKeyWords + '%' OR
		tfspjt0.customer_name LIKE '%' + @sKeyWords + '%' OR
		tfspjt0.prouct_code LIKE '%' + @sKeyWords + '%' OR
		tfspjt0.product_name LIKE '%' + @sKeyWords + '%' OR
		tfspjt0.asset_class_name LIKE '%' + @sKeyWords + '%' OR
		tfspjt0.currency_code LIKE '%' + @sKeyWords + '%' OR
		tfspjt0.amount LIKE '%' + @sKeyWords + '%' OR
		tfspjt0.rm_name LIKE '%' + @sKeyWords + '%' OR
		tfspjt0.remark LIKE '%' + @sKeyWords + '%')
END
