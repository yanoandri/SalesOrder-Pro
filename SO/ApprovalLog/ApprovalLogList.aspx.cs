...
public partial class SE_WebUI_ApprovalLogList : PFSBasePage
{
    ...
    protected void Page_Load(object sender, System.EventArgs e)
    {
        try
        {
            ...
            if (!IsPostBack)
            {
                if (!Security.CheckSecurity(PSCEnumeration.PFSModuleObjectMember.APP_LOG_READ.ToString()))
                    NoPermission();
                btnExportToExcel.Visible = Security.CheckSecurity(PSCEnumeration.PFSModuleObjectMember.APP_LOG_EXPORT_EXCEL.ToString());
            ...
            if (oApprovalLogList.Count > 0)
            {
                ...
                foreach (ApprovalLog oApprovalLog in oApprovalLogList)
                {
                    ...
                    if (oApprovalLogList.IndexOf(oApprovalLog) > 0)
                    {
                        ...
                        else if (oApprovalLog.ApprovalStatusID != (int)PSCEnumeration.ApprovalStatus.PENDING)
                        {
    ...
    private bool SingleApprovalProcess(int iModuleObjectID, int ApprovalLogID, int? oGroupObjectMember = null)
    {
        try
        {
            ...
            {
                case (int)PSCEnumeration.PFSModuleObject.User:
                    Response.Redirect("~/Security/UserInput.aspx?ID=" + ApprovalLogID);
                    break;

                case (int)PSCEnumeration.PFSModuleObject.Group:
                    if (oGroupObjectMember == (int)PSCEnumeration.PFSModuleObjectMember.SCR_GRP_ACCESS)
                ...
                case (int)PSCEnumeration.PFSModuleObject.Product:
                    Response.Redirect("~/Product/ProductInput.aspx?ID=" + ApprovalLogID);
                    break;
                case(int)PSCEnumeration.PFSModuleObject.ExceptionForm:
